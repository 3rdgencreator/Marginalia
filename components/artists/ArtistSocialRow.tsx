...
type IconLinkProps = { href: string; label: string; iconKey: string };

function IconLink({ href, label, iconKey }: IconLinkProps) {
  return (
    <a
      href={href}
      target="_blank"
...
export default function ArtistSocialRow({
  soundcloudUrl,
  spotifyUrl,
  beatportUrl,
  instagramUrl,
  ...
  const hasAny =
    soundcloudUrl || spotifyUrl || beatportUrl || instagramUrl ||
    residentAdvisorUrl || youtubeUrl || layloUrl;

  if (!hasAny) return null;

  return (
    <div className="flex flex-wrap items-center gap-1 -ml-3">
      {instagramUrl && <IconLink href={instagramUrl} label="Instagram" iconKey="instagram" />}
      {soundcloudUrl && <IconLink href={soundcloudUrl} label="SoundCloud" iconKey="soundcloud" />}
